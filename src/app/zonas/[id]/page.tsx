...
export default function ZonaSeguraDetallePage() {
  ...
  const [isUnsafeZone, setIsUnsafeZone] = useState(false); // Estado para el formulario
  ...
  return (
    <ProtectedRoute allowedRoles={["admin", "usuario"]}>
    <div className={styles.container}>
      <h1 className={styles.title}>Editar Zona: {zona.nombre}</h1>
          ...
          <p className={styles.mapInstructions}>
            ...
            {!loadingZones && zones && zones.length > 0 && (
              <span className={styles.existingZonesInfo}>
                <br />Zona actual mostrada en azul. Las demás zonas según su tipo: 🟢 seguras, 🔴 inseguras ({zones.length - 1} zona{zones.length !== 2 ? 's' : ''} adicional{zones.length !== 2 ? 'es' : ''}).
              </span>
            )}
            {loadingZones && (
