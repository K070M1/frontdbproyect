...
export default function ZoneForm({
  ...
  onFormStateChange?: (formState: { inseguro: boolean }) => void; // Nueva prop para notificar cambios de estado
  drawnShape?: any;
  ...
  useEffect(() => {
    if (editMode && initialData) {
      setForm({
        ...
        inseguro: initialData.inseguro || false,
      });
  ...
  // Notificar cambios de estado del formulario al componente padre
  useEffect(() => {
    if (onFormStateChange) {
      onFormStateChange({ inseguro: form.inseguro });
    }
  }, [form.inseguro, onFormStateChange]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!editMode && (!externalDrawnShape || !drawnShapeType)) {
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Debes dibujar una figura en el mapa antes de guardar la zona.',
      });
      return;
    }
    ...
    console.log(`ZoneForm: ${actionText} zona con datos:`, dataToSave);

    Swal.fire({
      title: `Confirmar ${editMode ? 'Actualización' : 'Guardado'}`,
      text: `¿Estás seguro de que deseas ${actionText} la zona "${form.nombre}"?`,
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (response) {
            ...
            Swal.fire({
              icon: 'success',
              title: `Zona ${editMode ? 'Actualizada' : 'Guardada'}`,
              text: `La zona ha sido ${actionPastText} exitosamente.`,
            });
            console.log(`Zona ${actionPastText}:`, response);
            router.push("/zonas");
          } else {
          ...
          Swal.fire({
            ...
            text: `No se pudo ${actionText} la zona. Inténtalo de nuevo.`,
          });
        }
      }
    });
  ...
  return (
    <form
      ...
      className={`${styles.form} ${form.inseguro ? styles.insegura :
          (!editMode ? styles.nueva : '')
        }`}
    >
      <h2 className={styles.formTitle}>
        {editMode ? 'Editar Zona' : 'Crear Zona'}
      </h2>
      ...
      <div className={styles.inputGroup}>
        ...
        {mounted && (
          <textarea
            ...
            onChange={handleChange}
            placeholder="Describe por qué este lugar es seguro o inseguro"
            rows={3}
            required
          />
        )}
        ...
        <label className={styles.checkboxLabel}>
          {mounted && (
            <input
              type="checkbox"
              name="inseguro"
              checked={form.inseguro}
              onChange={handleChange}
              className={styles.checkbox}
            />
          )}
          <span className={styles.checkboxText}>
            Marcar como zona insegura
          </span>
        </label>
        <p className={styles.checkboxHint}>
          Activa esta opción si la zona representa un área peligrosa o insegura
        </p>
      </div>
      ...
      <div className={styles.polygonSection}>
        ...
        <div className={styles.polygonOptions}>
          {["rectangle", "circle", "polygon"].map((type) => {
            // Determinar el color según el estado del formulario
            const getStrokeColor = () => {
              if (form.inseguro) return "#ef4444"; // Rojo para zonas inseguras
              if (!editMode) return "#6366f1"; // Índigo para nuevas zonas
              return "#10b981"; // Verde por defecto para edición
            };
            ...
            return (
              <button
                key={type}
                type="button"
                className={`${styles.polygonButton} ${selectedPolygon === type ? styles.selected : ""
                  } ${selectedPolygon === type ? styles[type] : ""} ${isDrawingMode ? styles.drawing : ""
                  }`}
