...
export default function RatingForm({ id_usuario }: RatingFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!ratingTarget || !ratingPosition) {
      Swal.fire({
        title: 'Error',
    ...
    // Construir datos según el modelo de base de datos
    const calificationData: any = {
      calificacion: form.calificacion,
      comentario: form.comentario,
      tipo_calificacion: ratingTarget.type, // "evento" o "zona"
    };
    ...
    try {
      Swal.fire({
        ...
        text: '¿Estás seguro de enviar esta calificación?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          const result = await addCalification(calificationData);
          console.log('Calificación enviada:', result);
          if(result){
            Swal.fire({
        ...
        <div className="space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button
              ...
              disabled={form.calificacion === 0 || !form.comentario.trim() || !form.referencia.trim()}
            >
              {ratingTarget ? "Enviar Calificación" : "Selecciona un punto en el mapa"}
            </Button>
          </form>
        </div>
      </div>
